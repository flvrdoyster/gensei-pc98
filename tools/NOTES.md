...
KR 텍스트에서 `/리`, `/프` 같이 슬래시 다음 한글 1자를 반각 한글로 처리.

```
'리프레시워터'  → 전각 6자 = 시각 폭 6 (슬롯 폭 5 초과)
'/리/프/레/시워터' → 반각 4자(폭 2) + 전각 2자(폭 2) = 시각 폭 4 (fit)
```

charmap.json에 `/리: 85a4` 형식으로 매핑. 인서터(`hukyou_inserter.encode_korean`,  
`kitan_inserter.encode_korean_kitan`)가 `/X` 시퀀스를 만나면 반각 코드로 인코딩.
...
- `0x85A4~0x85A7`: 리/프/레/시 (희담 리프레시워터용)
...
8×16 픽셀, **왼쪽 8픽셀에 한글 그리고 오른쪽 8픽셀은 비움**. 카페24 프로슬림 Fit  
14pt 기준으로 PIL 렌더 후 사용자가 픽셀 다듬기. 코드는 `/tmp/render_final.py` 류  
참고 (PIL TTF → 1bpp BMP 직접 비트 쓰기).
