...
**가이지 접미사 보존**:  
`battle` 태그 스킬명 일부는 `[SJIS 이름][가이지 코스트 0x85XX…]`가 구분자 없이 붙어 있음.  
원본 데이터는 `大撃剣` + `85 47 85 72 85 6F 85 50 85 4F 85 48`(`(SP10)` 가이지) 처럼 저장됨.  
파서는 `0x85`를 만나면 추출을 중단해 jp에는 스킬명만 담고 jp_len도 그 길이까지만 설정.  
인서터는 단순히 jp_len 범위만 패치하므로 가이지 영역은 자연스레 보존됨.
...
원본이 `0x85XX` 가이지로 저장된 텍스트(예: `リフレッシュの水` = 반각 6자 + 전각 2자 = **시각 폭 5**)는  
KR을 그냥 전각 한글로 번역하면(`리프레시워터` = 6자, 시각 폭 6) 슬롯 픽셀 폭을 초과해서  
다음 텍스트 위치를 밀어내거나 그리기 잔상이 남는 문제 발생.

인서터(`_compute_visual_width` + `_truncate_overwidth_kr`)가 자동으로:
- JP 바이트를 스캔해서 시각 폭 계산 (가이지·반각 ASCII = 0.5, 전각 SJIS = 1.0)
- KR이 그 폭을 넘으면 char 단위로 trim 후 경고 출력

번역자는 그냥 자연스러운 한글로 적되, 폭 초과 시 더 짧은 표현(예: `재생수`)으로 대체하면 됨.

### 반각 한글 마커 (실험적, 현재 미사용)

KR 텍스트에서 `/X` 형태로 시작하는 두 글자는 **반각 한글**로 해석 (charmap에 `/리`처럼 매핑된 경우).  
인서터(`encode_korean_kitan`)가 `/리` → `0x9540` 식으로 인코딩, 폭 계산도 0.5로 처리.

**시도 결과**: 희담은 텍스트 렌더링 시 `0x95XX` 영역을 그냥 전각 한자로 그려서 반각 효과 안 남.  
풍광전에서는 가능할지도 모름 (검증 필요).

현재 charmap에 등록된 반각 매핑: `/리`(0x9540), `/프`(0x9541), `/레`(0x9542), `/시`(0x9543)  
font.bmp의 해당 셀(656,528~)에 반각 글리프 그려져 있음.
